...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeq8ecoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeq8ecoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeq8ecoinAmount = strAnonymizeq8ecoinAmount.remove(strAnonymizeq8ecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeq8ecoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeq8ecoinAmount*COIN) nMaxToAnonymize = nAnonymizeq8ecoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeq8ecoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeq8ecoinAmount));
        strAnonymizeq8ecoinAmount = strAnonymizeq8ecoinAmount.remove(strAnonymizeq8ecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeq8ecoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
