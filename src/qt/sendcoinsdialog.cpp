    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Q8E address (e.g. Q8E_Address_Here_QsAqFckyf9pRHN)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        Cq8ecoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to 1000 Q8E.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        Cq8ecoinAddress addr = Cq8ecoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Q8E address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    ui->labelCoinControlAutomaticallySelected->setStyleSheet("QLabel{color:white;}");
    ui->labelCoinControlInsuffFunds->setStyleSheet("QLabel{color:white;}");
    ui->widgetCoinControl->setStyleSheet("QLabel{color:white;}");
