...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeq8ecoinAmount"))
        settings.setValue("nAnonymizeq8ecoinAmount", 1000);
    nAnonymizeq8ecoinAmount = settings.value("nAnonymizeq8ecoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeq8ecoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeq8ecoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case Anonymizeq8ecoinAmount:
            return QVariant(nAnonymizeq8ecoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case Anonymizeq8ecoinAmount:
            nAnonymizeq8ecoinAmount = value.toInt();
            settings.setValue("nAnonymizeq8ecoinAmount", nAnonymizeq8ecoinAmount);
            emit Anonymizeq8ecoinAmountChanged(nAnonymizeq8ecoinAmount);
            break;
        default:
            break;
        }
